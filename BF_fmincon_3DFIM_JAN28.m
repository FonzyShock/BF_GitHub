...
data.v_T   = [10, 0, 0];                  % m/s, target velocity (known)
...
data.kappa = 1e-3;                    % Barrier parameter (tunable)
...
data.N_B = 50;                        % Bernstein polynomial order
data.N_F = 10;                        % Fourier series order
...
% --- Plot Initial Guess ---
figure('Name', 'Initial_Guess_Check', 'Position', [200, 200, 700, 500]);
hold on; grid on; axis equal;
valid_idx = 1:data.Nt_BF;
plot(Cx_BF(valid_idx, 1), Cx_BF(valid_idx, 2), 'b.-', 'LineWidth', 1, 'MarkerSize', 12, 'DisplayName', 'UAV Guess (Bernstein Nodes)');
t_guess = data.time_BF * data.tf;
X_T_guess = data.x_T0(1) + data.v_T(1) * t_guess;
Y_T_guess = data.x_T0(2) + data.v_T(2) * t_guess;
plot(X_T_guess, Y_T_guess, 'r--', 'LineWidth', 1.5, 'DisplayName', 'Target Path');
t_app_val = norm(data.x_T0(1:2) + [-data.r_NFZ, 0]) / data.V_A;
dist_app  = data.V_A * t_app_val;
plot(dist_app, 0, 'kx', 'MarkerSize', 10, 'LineWidth', 2, 'DisplayName', 'Switch to Orbit');
xlabel('X [m]'); ylabel('Y [m]');
title('Initial Guess Validation: Approach & Moving Orbit');
...
data.N_bern = 60;
data.r_NFZ_bern = 100; % Relaxed constraint for baseline
% Time nodes
...
yline(data.r_NFZ, 'r--', 'DisplayName', 'NFZ');
...
function J = costFunc_Bernstein_MovingTarget_3DFIM(X, data)
    ...
    %     end
    % end
    %CRLB_trace = CRLB_cumulative(end);
    barrier_term = kappa * norm(max(0, r_NFZ - rxy))^2;
